...
Scheduled jobs and installers that run the workspace. Seven launchd agents (clip
ingestion, Friday close-out, Monday note init, health check, skill sync, closed-project pickup, decisions sweep) plus their scripts. All
...
| `notify.sh` | Raise a desktop notification on this machine: `bash System_Config/notify.sh "<title>" "<body>"`, or the event form `--event <id> --severity info|ok|warn|fail --dedup <sig>`. macOS Notification Center via `osascript`, `notify-send` elsewhere — local delivery is unconditional and the default, needing no credentials or network. Webhook delivery (Slack, Google Chat) is additive and strictly opt-in via `.notify.env` (`GCHAT_WEBHOOK_URL` / `SLACK_WEBHOOK_URL`); the URL is piped to `curl --config -` on stdin so it never appears in `ps` output. Exits 0 when **any** configured channel delivered, or delivery was deliberately suppressed (`NOTIFY_ENABLED=0`, or an unchanged `--dedup` signature); exits 1 only when every configured channel failed, leaving the dedup signature unwritten so the alert retries. Each channel's outcome is logged independently regardless of the overall exit code. Called by `healthcheck.sh` only when the **set** of non-PASS check names changes (signature in `logs/.notify_state`), which is persisted only after a successful notification — so a stable warning stays quiet, recovery is announced, and a failed alert comes back. Payloads carry counts and check names only — never vault content. |
| `.notify.env` | **Git-ignored** local settings for `notify.sh` and `spend.sh`. Entirely optional — the notifier works with no config at all. Copy from `.notify.env.example`, or let `bootstrap.sh`'s notification picker create it. Holds `NOTIFY_ENABLED` (set `0` to silence every notification), the opt-in `GCHAT_WEBHOOK_URL` / `SLACK_WEBHOOK_URL`, and `SPEND_ALERT_USD`. An explicitly exported variable overrides the file for that one invocation. |
...
| `decisions_sweep.sh` | Twice-daily sweep of THIS week's note for undocumented decisions (extracted from `friday_process.sh`'s step C, which still runs as the Friday backstop). Two-tier "free first" design, a SEPARATE mechanism from `run_agent.sh`'s own `INGEST_TARGETS=ollama` path (which shells out to `codex exec --oss --local-provider ollama`): tries a standalone Ollama tool-calling loop (`ollama_agent.py`, direct HTTP to Ollama's local API) first; on failure or a confirmed no-op (hash of the note's `## Decisions` slice unchanged — not the whole file, since other jobs write elsewhere in the same note concurrently) it escalates **once** to a paid claude/codex call via `resolve_agent_target()` + `run_agent()`. `DRY_RUN=1` support; serialized behind `logs/decisions_sweep.lock`. |
| `ollama_agent.py` | Stdlib-only Ollama tool-calling loop used directly by `decisions_sweep.sh`'s free tier (read/list/grep/write/append tools, path-confined to the vault, write deny-list). `--selftest` runs standalone, no live Ollama server required. |
| `decisionssweep.plist.tmpl` | launchd agent template: runs the sweep twice daily (`DECISIONSSWEEP_NOON_*` / `DECISIONSSWEEP_EVENING_*`, default 12:00 + 18:00). |
| `install_decisions_sweep.sh` | Render + install/reload the decisions-sweep agent (idempotent). |
