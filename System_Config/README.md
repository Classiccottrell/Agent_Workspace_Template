...
| `notify.sh` | Raise a desktop notification on this machine: `bash System_Config/notify.sh "<title>" "<body>"`, or the event form `--event <id> --severity info|ok|warn|fail --dedup <sig>`. macOS Notification Center via `osascript`, `notify-send` elsewhere — no webhook, no credentials, no network, nothing to configure before it works. Exits 0 when a notification was raised **or** deliberately suppressed (`NOTIFY_ENABLED=0`, or an unchanged `--dedup` signature); exits 1 when delivery was attempted and failed, leaving the dedup signature unwritten so the alert retries rather than silencing itself. Called by `healthcheck.sh` only when the **set** of non-PASS check names changes (signature in `logs/.notify_state`), which is persisted only after a successful notification — so a stable warning stays quiet, recovery is announced, and a failed alert comes back. Payloads carry counts and check names only — never vault content. |
| `.notify.env` | **Git-ignored** local settings for `notify.sh` and `spend.sh`. Entirely optional — the notifier works with no config at all. Copy from `.notify.env.example`. Holds `NOTIFY_ENABLED` (set `0` to silence every notification) and `SPEND_ALERT_USD`. An explicitly exported variable overrides the file for that one invocation. |
