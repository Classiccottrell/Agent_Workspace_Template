...
* Routing: architecture/schema → `architect`; implementation → `coder`; `Projects/` lifecycle → `eng-manager`; `Closed/` project closure → `archivist`; `Vault_Brain/` knowledge → `curator`; brand/visual/copy/design-feedback → Claude Code's `creative-director`; production-repo PR regression/QA → `qa`.
* Codex agent scopes are advisory instructions; Codex currently has no per-agent enforced tool allow-list.
