...
3. Apply naming: `YYYY-MM-DD_<artifact-type>.<ext>` inside `Final_Products/<project-name>/`.
4. Maintain the master `Final_Products/INDEX.md` (project registry) and each project's `<project>/INDEX.md` (artifact manifest), updated on every archive event.
...
Context discipline:
- Index before reading: `find Final_Products/ -maxdepth 2 -name '*.md' | sort`.
- Use `ls Final_Products/<project-name>/` to index one project's artifacts before reading.
